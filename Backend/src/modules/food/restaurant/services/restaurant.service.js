...
import { getRestaurantOnboardingSettings } from '../../admin/services/admin.service.js';
...
export const createRestaurantOnboardingFeeOrder = async ({ ownerPhone }) => {
    const settings = await getRestaurantOnboardingSettings();
...
export const registerRestaurant = async (payload, files) => {
    ...
    const onboardingSettings = await getRestaurantOnboardingSettings();
    const requiredOnboardingFee = Math.max(0, Number(onboardingSettings?.onboardingFee) || 0);
