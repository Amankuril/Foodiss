...
export async function getRestaurantOnboardingSettings(req, res, next) {
    try {
        const data = await adminService.getRestaurantOnboardingSettings();
        res.status(200).json({ success: true, message: 'Onboarding settings fetched successfully', data });
    } catch (error) {
        next(error);
    }
}

export async function updateRestaurantOnboardingSettings(req, res, next) {
    try {
        const data = await adminService.updateRestaurantOnboardingSettings(req.body);
        res.status(200).json({ success: true, message: 'Onboarding settings updated successfully', data });
    } catch (error) {
        next(error);
    }
}
